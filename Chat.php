...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['delete'])) {
        ...
        if (!empty($message)) {
            $sql = "INSERT INTO chat_messages (email, message, created_at) VALUES (?, ?, NOW())";
            $stmt = $conn->prepare($sql);
...
$sql = "SELECT cm.id, cm.message, u.username, cm.created_at FROM chat_messages cm JOIN users u ON cm.email = u.email ORDER BY cm.created_at ASC";
...
<head>
    ...
    <style>
        ...
        .bg {
            ...
            background-image: linear-gradient(to bottom left, #645FCE 40%, #00A5CF 40%);
            bottom:0;
            left:-50%;
            opacity:.5;
            position:fixed;
