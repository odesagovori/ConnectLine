...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($message)) {
        $sql = "INSERT INTO algorithms_chat (email, message, created_at) VALUES (?, ?, NOW())";
        $stmt = $conn->prepare($sql);
        $stmt->execute([$userId, $message]);
    }
}

$sql = "SELECT cm.message, u.username, cm.created_at FROM algorithms_chat cm JOIN users u ON cm.email = u.email ORDER BY cm.created_at ASC";
...
<head>
    ...
    <style>
        ...
        .bg {
            ...
            background-image: linear-gradient(to bottom left, #004E64 40%, #00A5CF 40%);
            bottom:0;
            left:-50%;
            opacity:.5;
            position:fixed;
...
<body>
    ...
    <h1 class="chatroom-heading">Hyrje në Algoritme Chat Room</h1>
